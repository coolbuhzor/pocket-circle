      ...
      {members.map((member, index) => {
        ...
        return (
          <div
            key={member.id}
            className={cn(
              "absolute flex items-center justify-center rounded-full font-semibold text-white shadow-sm transition-transform duration-500",
              font,
              isCollector
          ...
          >
            {isCollector && (
              <span
                className="absolute inset-0 rounded-full bg-accent/60 animate-[pc-ring_2.6s_ease-out_infinite]"
                aria-hidden
              />
            )}
            <span className="relative">{getInitials(member.name)}</span>
          </div>
        );
      })}
    </div>
